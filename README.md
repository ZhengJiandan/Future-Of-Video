...
- 展示地址：[http://82.156.153.123/](http://82.156.153.123/)
- 用户：test，密码：123456
- 仅供展示使用。
...
- 已安装 Node.js 18+
- 已安装 FFmpeg
- 如需直接使用豆包能力，已准备 `DOUBAO_API_KEY`
- 已复制 `backend/.env.example` 到 `backend/.env`

注意：

- 当前后端已经支持“无 `DOUBAO_API_KEY` 先启动”；只有真正调用剧本生成、图片分析、豆包视频生成等能力时，才会校验 key。
- 如果服务端没配 `DOUBAO_API_KEY`，前端也可以在当前浏览器会话里临时填写后继续调用。
...
- `KLING_SECRET_KEY`
  - 用于可灵视频生成。
  - `provider=auto` 时，如果这两个配置都存在，会优先使用可灵；如果缺失，则自动降级到豆包视频模型。
- `DOUBAO_API_KEY`
  - 服务端可预先配置；如果未配置，也可在前端会话里临时填写后使用豆包能力。
...
### 4. 为什么现在不配 `DOUBAO_API_KEY` 也能启动？

因为当前实现已经改为懒校验。后端启动时不会再因为缺少 `DOUBAO_API_KEY` 直接失败；只有真正调用这些能力时才会检查：
...
- 如果已配置 `KLING_ACCESS_KEY` + `KLING_SECRET_KEY`，`provider=auto` 时优先走可灵视频生成
- 如果没有配置可灵视频凭证，则自动降级到豆包视频模型
- 如果两边都没有可用凭证，则会报错，或者你也可以显式切到 `local` 预览模式

### 8. 临时角色什么时候会提示保存到角色档案库？
